...
- **Packer**：新增「调试」标签页（GUI-only）——两种模式：**调试源码**（运行
  `[tool.dghub].entry` 声明的源码，注入 `DGHUB_MANIFEST_DIR=插件目录/.dghub-sdk`，
  用户插件代码零改动）与**调试运行**（先构建再运行产物，构建始终文件夹输出、
  固定到 `插件目录/debug/`）；环境变量区支持 DGHub 本机探测与手动填写
  （主机/端口/令牌，令牌由用户提供）；插件 stdout/stderr 进日志页，支持
  停止（终止进程树）
- **Packer**：编译系统新增调试能力 API（`debug_source_command`——返回
  调试源码启动命令，不支持返回 None；Python 编译支持，其余默认不支持），
  调试页不内置编译系统知识
