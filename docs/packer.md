...
- **文件夹模式 (No Zip)** — 复选框：不勾 = `.zip` 包（默认，分发）；勾选 = 输出文件夹（本地调试）
...
- **DGHub 主机 / 端口** — 调试运行时注入插件子进程的主机与端口（默认 localhost:8000）；检测 DGHub 成功后自动回填
- 应用版本信息、相关链接与开源协议（AGPLv3）

### 日志

构建全过程输出集中显示：校验、compile、依赖安装、PyInstaller、打包逐步记录；校验失败与构建错误在此给出具体原因；旧版配置迁移等提示也记录于此。**调试运行**的插件 stdout/stderr 同样进入本页（`─── 调试源码 ───` / `─── 调试运行 ───` 分隔块）。
...
在插件目录内运行插件，验证与 DGHub 服务端的交互（GUI-only，CLI 不含调试命令）：

- **调试模式** — 下拉选择：
  - **调试源码** — 运行 `[tool.dghub].entry` 声明的源码（`uv run --project`，依赖环境由项目自身提供）；仅编译系统支持时可用（Python 编译支持；其他编译系统提示「编译系统 '<系统名>' 不支持「调试源码」」）；注入 `DGHUB_MANIFEST_DIR=插件目录/.dghub-sdk`（manifest 读取开发态数据，用户插件代码零改动）
  - **调试运行** — 先构建再运行产物：构建始终输出文件夹（与项目发布目标配置无关）、固定到 `插件目录/debug/`，然后在产物文件夹内运行插件入口（Python 编译为 `<插件名>.exe`）；产物结构与分发 folder 一致
- **环境变量** — 令牌（DGHUB_TOKEN）：
  - 主机与端口在「设置」页全局配置（不常变动），调试页仅保留令牌字段
  - 「检测 DGHub」探测本机 DGHub 连通性，成功自动回填主机/端口到设置页并拉取当前会话令牌
  - 令牌由用户填写或沿用环境变量；检测成功自动填入——令牌是 DGHub 服务端发放的凭据，每次重启更换
- **启动 / 停止** — 「启动调试」后台运行插件，stdout/stderr 进日志页；「停止」终止进程树（含子进程）；调试运行期间构建按钮禁用（互斥）
- 插件为窗口程序时窗口正常显示；windowed 产物（无控制台）无 stdout，日志仅显示启动与退出状态
