...
- **入口文件** — 阶段 1 输入：Python 编译时为 `.py` 源码入口；无编译/自定义命令时为产物路径；`[tool.dghub].entry` 约定可由「从编译填充」自动应用
...
- 校验一次性检测所有必填项（信息页 id / name / version、入口文件、编译必要字段、打包内容入口条目、输出目录），错误一次全部高亮；修正任一字段即刻恢复其边框，该页错误全部清除后标题高亮与「构建失败」状态一并消除
