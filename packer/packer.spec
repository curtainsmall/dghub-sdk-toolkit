...
"""PyInstaller 构建配方：onedir + MERGE，GUI 与 CLI 共享一份运行时。

产出单个文件夹 `dghub-sdk-packer/`，含两个启动器：
  - `dgpacker-gui.exe`（windowed，GUI）
  - `dgpacker.exe`（console，CLI；安装后即控制台命令 `dgpacker`）
...
exe_cli = EXE(
    ...
    strip=False,
    upx=True,
    console=True,                # console CLI，stdout 可见
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
